...
openstack subnet create public-subnet \
	...
	--gateway 192.168.0.1 \
	--allocation-pool start=192.168.50.125,end=192.168.50.200 \
...
[Cinder 볼륨 연동형 VM 생성]
디스크는 sto01 노드에 생성되고 Compute 노드에는 CPU와 MEM만 존재하는 형태의 VM을 만드는 명령어
```
openstack volume create \
--size 30 \
--image cirros-0.6.3-aarch64 \
--description "TestVM" \
--bootable \
--read-write \
cirros-test01-vol01
...
--volume cirros-test01-vol01 \
--flavor o1.default \
--security-group allowall \
--network VMNET01 \
--user-data ogjs-userdata.txt \
cirros-test01
```

[로컬 디스크 VM 생성]
ODROID 구성 환경은 네트워크가 느리기 때문에(현재는 오드로이드 아님), VM이 생성되는 곳에 디스크를 만들면 성능면에서 좀 더 낫다. 물론, 서버가 죽어버리면 VM도 같이 죽어버리는 단점있음
```
openstack server create \
--flavor o1.default \
--image cirros-0.6.3-x86_64 \
--availability-zone nova:os-compute \
--security-group allowall \
--network VMNET01 \
--user-data ogjs-userdata.txt \
test01
...
2. OVS 브리지 매핑 : `/etc/neutron/plugins/ml2/openvswitch_agent.ini`
	...
	```
	- `openvswitch_agent.ini`의 [ovs] → OVS ML2 메커니즘 드라이버용
	- `ml2_conf.ini`의 [ovn] → OVN ML2 메커니즘 드라이버용
	  (`openvswitch_agent.ini`에 있는 `bridge_mappings`는 OVN 환경에서는 무시되므로,  
→ `/etc/neutron/plugins/ml2/ml2_conf.ini` 안의 [ovn] 섹션으로 옮겨야 됨)
3. Neutron agent 재시작
